...
function provisionStatus(provision: Provision, ledger: Ledger, reference: IsoDate): ProvisionStatusV2 {
  ...
  const beforeMonth = movements.filter((m) => monthOf(m.date) < refMonth)
  const thisMonth = movements.filter((m) => monthOf(m.date) === refMonth)
  const balanceBeforeMonth = beforeMonth.reduce((sum, m) => sum + (m.kind === 'depot' ? m.amount : -m.amount), 0)
  const fundedBeforeMonth = Math.max(0, provision.initial_amount + balanceBeforeMonth)
  const fundedThisMonth = thisMonth.reduce((sum, m) => sum + (m.kind === 'depot' ? m.amount : -m.amount), 0)
  const funded = Math.max(0, fundedBeforeMonth + fundedThisMonth)
  const remaining = Math.max(0, provision.target_amount - funded)

  const remainingAtMonthStart = Math.max(0, provision.target_amount - fundedBeforeMonth)
  const monthsLeft = provision.target_date < reference
    ? 0
    : Math.max(1, monthsBetween(refMonth, targetMonth) + 1)
  const monthlyNeeded = monthsLeft > 0 ? Math.ceil(remainingAtMonthStart / monthsLeft) : remainingAtMonthStart
...
export function computeMonthV2(ledger: Ledger, month: IsoMonth, reference: IsoDate): DisciplineSnapshot {
  ...
  const incomeExpected = monthIncomes.filter((i) => i.date > reference).reduce((sum, i) => sum + i.amount, 0)
  const provisions = computeProvisionsV2(scoped, reference)

  const provisionPocketIds = new Set(
    provisions.map((p) => p.pocketId).filter((id): id is string => Boolean(id)),
  )
  const generalSavingsDone = alive(scoped.savings)
    .filter((m) => monthOf(m.date) === month && !provisionPocketIds.has(m.pocket_id))
    .reduce((sum, m) => sum + (m.kind === 'depot' ? m.amount : -m.amount), 0)
  const generalSavingsRemaining = Math.max(0, base.savingsTarget - Math.max(0, generalSavingsDone))

  const provisionsReserveRemaining = monthOf(reference) === month
    ? provisions.reduce((sum, p) => sum + p.reserveRemainingThisMonth, 0)
    : 0
  const protectedReserveRemaining = generalSavingsRemaining + provisionsReserveRemaining

  // Toute epargne deja versee est une vraie sortie de tresorerie. Les reserves
  // restantes sont ensuite ajoutees separement : epargne generale + provisions.
  // Un versement sur la poche d'une provision reduit sa reserve sans reduire
  // artificiellement deux fois la tresorerie.
  const available = base.income - base.spent - base.savingsDone - base.chargesRemaining - protectedReserveRemaining
  const spendable = Math.max(0, available)
  const deficit = Math.max(0, -available)

  const todaySpent = monthOf(reference) === month
    ? scoped.expenses.filter((e) => e.deleted_at === null && e.date === reference).reduce((sum, e) => sum + e.amount, 0)
    : 0
  ...
  const envelopeAllocated = base.envelopes.reduce((sum, e) => sum + e.planned, 0)
  const provisionTarget = provisions.reduce((sum, p) => sum + p.monthlyNeeded, 0)
  const protectedMonthlyTarget = base.savingsTarget + provisionTarget
  ...
  if (available < 0) {
    ...
    health = 'saine'
    healthReason = 'Vos charges, votre epargne, vos provisions et vos depenses tiennent dans vos revenus encaisses.'
  }
  ...
  const withCore = {
    ...
    savingsRemaining: generalSavingsRemaining,
    available,
    spendable,
    deficit,
    dailyBudget,
...
export function computeScoreV2(
  ledger: Ledger,
  snapshot: Omit<DisciplineSnapshot, 'score'>,
  reference: IsoDate,
): DisciplineScoreV2 {
  const parts: ScoreComponentV2[] = []
  const planned = snapshot.envelopes.reduce((sum, e) => sum + e.planned, 0)
  const over = snapshot.envelopes.reduce((sum, e) => sum + Math.max(0, e.spent - e.planned), 0)
  parts.push(planned > 0
    ? component(
        'budgets',
        'Respect des budgets',
        25,
        Math.round(25 * (1 - Math.min(1, over / planned))),
        true,
  ...
  const provisionTarget = snapshot.provisions.reduce((sum, p) => sum + p.monthlyNeeded, 0)
  const protectedTarget = snapshot.savingsTarget + provisionTarget
  parts.push(protectedTarget > 0
    ? component(
        'epargne',
        'Epargne et provisions realisees',
        20,
        Math.round(20 * Math.min(1, Math.max(0, snapshot.savingsDone) / protectedTarget)),
        true,
        `${formatInt(Math.max(0, snapshot.savingsDone))} FCFA proteges sur ${formatInt(protectedTarget)} FCFA vises.`,
        snapshot.savingsDone >= protectedTarget
          ? 'Objectif atteint.'
          : `Il reste ${formatInt(Math.max(0, protectedTarget - snapshot.savingsDone))} FCFA a proteger.`,
      )
    : component('epargne', 'Epargne et provisions realisees', 20, 0, false, "Aucun objectif d'epargne ou provision mesurable.", 'Definissez un taux ou une provision.'))
  ...
  parts.push(due.length > 0
    ? component(
        'charges',
        'Charges payees a temps',
        15,
        Math.round(15 * onTime.length / due.length),
        true,
  ...
  if (expenses.length > 0) {
    ...
    const unplanned = expenses
      .filter((e) => e.envelope_id === null && e.charge_id === null)
      .reduce((sum, e) => sum + e.amount, 0)
    parts.push(component(
      'planification',
      'Depenses planifiees',
      15,
      Math.round(15 * (1 - Math.min(1, unplanned / Math.max(1, total)))),
      true,
    ...
    parts.push(component(
      'safe_to_spend',
      'Respect du Disponible a depenser',
      15,
      earned,
      true,
      danger === 0 && warning === 0
        ? 'Aucune depense forcee contre une alerte globale.'
        : `${danger} depense(s) en zone danger et ${warning} avertissement(s).`,
  ...
  parts.push(snapshot.daysElapsed > 0 && (snapshot.income > 0 || expenses.length > 0)
    ? component(
        ...
        'Regularite du suivi',
        10,
        Math.round(10 * Math.min(1, (days.size / snapshot.daysElapsed) / 0.6)),
        true,
        `${days.size} jour(s) avec saisie sur ${snapshot.daysElapsed} jour(s) observes.`,
        days.size / Math.max(1, snapshot.daysElapsed) >= 0.6
          ? 'Votre suivi est regulier.'
          : 'Saisissez les depenses au fil de la journee.',
      )
  ...
  const earned = applicable.reduce((sum, p) => sum + p.earned, 0)
...
export function simulateExpenseV2(
  ledger: Ledger,
  month: IsoMonth,
  reference: IsoDate,
  input: { amount: number; envelopeId: string | null; date: IsoDate },
): ExpenseSimulationV2 {
  ...
  if (input.amount <= 0) {
    return {
      ...
      availableAfter: before.available,
      dailyBefore: before.dailyBudget,
      dailyAfter: before.dailyBudget,
      envelope: null,
      globalRisk: 'none',
      flags: [],
      reasons: [],
  ...
  const synthetic: Expense = {
    id: '__simulation__',
    updated_at: `${reference}T00:00:00.000Z`,
    deleted_at: null,
    date: input.date,
    amount: input.amount,
    envelope_id: input.envelopeId,
    method: 'autre',
    description: '__simulation__',
    member: 'Moi',
    charge_id: null,
    override_reason: '',
    discipline_flags: [],
  ...
  const envelope = checkOverspend(
    before as unknown as MonthSnapshot,
    input.envelopeId,
    input.amount,
    ledger.settings.warn_threshold_pct,
  )
