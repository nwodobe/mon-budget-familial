    ...
    <>
      <Card title="À préparer">
        <p className="tiny" style={{ marginTop: 0 }}>
          Anticipez les grosses dépenses avant leur échéance. Une provision liée à une poche déjà alimentée n'est jamais déduite deux fois.
        </p>
        <div className="rows">
          <Row k="À provisionner ce mois" v={<Money value={snapshot.provisionsReserveRemaining} />} />
          <Row k="Réserve protégée globale" v={<Money value={snapshot.protectedReserveRemaining} />} note="Le système retient le besoin le plus exigeant entre épargne et provisions." />
        </div>
      </Card>

      <Card title="Préparations actives" action={<button className="btn small ghost" onClick={() => setAdding(true)}>Ajouter</button>}>
      ...
      {activeRows.length === 0 && (
        <Card title="Exemples">
          <div className="chips">
            {SUGGESTIONS.map((s) => <button key={s} className="chip" onClick={() => { setAdding(true) }}>{s}</button>)}
          </div>
        </Card>
      )}
      ...
      {adding && (
        <ProvisionSheet
          ...
          initial={{ name: '', target_amount: 0, target_date: today, pocket_id: null, initial_amount: 0, active: true }}
          onClose={() => setAdding(false)}
          onSave={(row) => { create('provisions', row); setAdding(false) }}
        />
      )}
...
function ProvisionSheet({ title, initial, onClose, onSave, onDelete }: {
  ...
  return (
    <Sheet title={title} onClose={onClose}>
      ...
      <Field label="Poche d'épargne dédiée" hint="Si vous versez déjà dans cette poche, le moteur le reconnaît et ne réserve pas la même somme deux fois.">
